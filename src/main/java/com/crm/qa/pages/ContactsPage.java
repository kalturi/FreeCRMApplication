...
public class ContactsPage extends TestBase {
	...
	public List<WebElement> getAlltheDataFromRow(String name) {
		...
		int count=0;
		 ...
		 for (WebElement a : arr) {
			 ...
			 if(name.equalsIgnoreCase(value)) {
				rowvalues= driver.findElements(By.xpath("//table[contains(@class,'custom-grid')]/tbody/tr["+count+"]"));
				 break;
			 }
	 ...
	 public void ClickOnNewContactsButton(String fname,String lname,String cat,String statusdrop,String Email,String phnNumber) throws InterruptedException	{
		 Thread.sleep(2000);
		...
		List<WebElement> catogeries= driver.findElements(By.xpath("//div[contains(@class,'visible menu transition')]"));
	...
	public void DeleteContact(String name) throws InterruptedException {
		...
		// driver.findElement(By.xpath("//div[contains(@class,'custom-pagination')]/a[3]")).click();
		for(int i=3;i<=pageCount;i++) {
			 System.out.println("In***********LOOP");
				...
				driver.navigate().refresh();
				 List<WebElement> arr1=driver.findElements(By.xpath("//table[contains(@class,'custom-grid')]/tbody/tr/td[2]"));
					for(WebElement x1: arr1) {
		 ...
		 /*for (WebElement ax : arr) {
				System.out.println(ax.getText());
				if(!arr.contains(name)) {
					for(int i=3;i<=pageCount-1;i++) {
						driver.findElement(By.xpath("//div[contains(@class,'custom-pagination')]/a['"+i+"']")).click();
						arr2=driver.findElements(By.xpath("//table[contains(@class,'custom-grid')]/tbody/tr/td[2]"));
						if(arr2.contains(name)) {
						if(ax.getText().equalsIgnoreCase(name)) {	
							driver.findElement(By.xpath("//a[text()='"+name+"']/parent::td/preceding-sibling::td/div")).click();
							driver.findElement(delContactBtn).click();
